...
const ct = {
     ...
     */
    version: '1.4.2',
    meta: [{"name":"Space Shooter","author":"kenliten","site":"https://kenliten.github.io/SpaceShooter/","version":"1.0.0"}][0],
    main: {
        fpstick: 0,
        pi: 0
    },
...
ct.inputs.addAction('Shoot', [{"code":"mouse.Left"},{"code":"keyboard.Space"}]);
...
ct.rooms.templates['MainRoom'] = {
    name: 'MainRoom',
    width: 800,
    height: 600,
    /* JSON.parse allows for a much faster loading of big objects */
    objects: JSON.parse('[{"x":384,"y":576,"type":"PlayerShip"}]'),
    bgs: JSON.parse('[{"depth":-5,"texture":"purple","extends":{}}]'),
...
if (this.asteroidTimer <= 0){
    ...
    if (this.lapse > 2000){
        this.asteroidTimer = ct.random.range(40, 200);
    }
    if (this.lapse > 3000){
        this.asteroidTimer = ct.random.range(30, 150);
    }
    if (this.lapse > 4000){
        this.asteroidTimer = ct.random.range(20, 100);
    }
    if (this.lapse > 5000){
        this.asteroidTimer = 40;
    }
    ct.types.copy(ct.random.dice('BigMeteor', 'MedMeteor'), ct.random(ct.camera.width), -100);
}
...
if (this.enemyTimer <= 0){
    ...
    if (this.lapse > 2000){
        this.enemyTimer = ct.random.range(40, 200);
    }
    if (this.lapse > 3000){
        this.enemyTimer = ct.random.range(30, 150);
    }
    if (this.lapse > 4000){
        this.enemyTimer = ct.random.range(20, 100);
    }
    if (this.lapse > 5000){
        this.enemyTimer = 40;
    }
...
ct.types.templates["PlayerShip"] = {
    depth: 0,
    texture: "playerShip1_blue",
    onStep: function () {
        // This should move the ship on the X axis
this.x += 8 * ct.delta * ct.actions.MoveX.value;
this.y += 8 * ct.delta * ct.actions.MoveY.value;
...
if (this.y < Math.round(ct.camera.height * .25)){
    this.y = Math.round(ct.camera.height * .25);
}
...
if (hostile){
    ...
    if (ct.room.lives <= 0){
        this.kill = true;
        setTimeout(function(){
            ct.rooms.switch('MainRoom');
        }, 100);
    }
}

var collided = ct.place.meet(this, this.x, this.y, 'PowerUp');
...
ct.types.list['PlayerShip'] = [];
ct.types.templates["EnemyShip"] = {
    depth: 0,
    texture: "enemyRed1",
    onStep: function () {
        if (this.x < 0){
    this.x = 0;
    this.direction = ct.random.range(270, 315);
}

if (this.x > ct.camera.width){
    this.x = ct.camera.width;
    this.direction = ct.random.range(180, 270);
}
...
if (this.bulletTimer <= 0){
    this.bulletTimer = ct.random.range(60, 180);
...
ct.types.list['EnemyShip'] = [];
ct.types.templates["EnemyLaser"] = {
    depth: 0,
    texture: "laserRed09",
    onStep: function () {
...
ct.types.list['EnemyLaser'] = [];
ct.types.templates["PlayerLaser"] = {
    depth: 0,
    texture: "laserBlue01",
    onStep: function () {
...
ct.types.list['PlayerLaser'] = [];
ct.types.templates["BigMeteor"] = {
    depth: 0,
    texture: "meteorGrey_big1",
    onStep: function () {
        if (this.y > ct.viewHeight + 80){
    this.kill = true;
}
...
if (collided){
    collided.kill = true;
    this.kill = true;
    ct.types.copy('MedMeteor', this.x, this.y);
    ct.types.copy('MedMeteor', this.x, this.y);
    ct.room.score += 40;
}
...
ct.types.list['BigMeteor'] = [];
ct.types.templates["MedMeteor"] = {
    depth: 0,
    texture: "meteorGrey_med1",
    onStep: function () {
        if (this.y > ct.viewHeight + 80){
    this.kill = true;
}
...
ct.types.list['MedMeteor'] = [];
ct.types.templates["PowerUp"] = {
    depth: 0,
    texture: "powerupGreen_bolt",
    onStep: function () {
        if (this.y > ct.viewHeight + 40){
    this.kill = true;
}
...
this.move();
    ...
    onCreate: function () {
        this.speed = 3;
this.direction = ct.random.range(225, 315);
    },
    extends: {}
};
ct.types.list['PowerUp'] = [];
